...
# Применяем nest_asyncio, чтобы избежать ошибок с уже запущенным event loop
nest_asyncio.apply()

# Запускаем dummy HTTP-сервер (Flask) в отдельном потоке,
...
# Загружаем модель явно на CPU через device_map="cpu"
